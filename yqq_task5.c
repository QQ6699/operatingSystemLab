...
//The second parameter is the semaphores set number
  ...
  void put_away_forks(int no,int semid)
  {
      ...
      int right = (no + 1) % 5;
      struct sembuf buf[2] = {
          {left,1,SEM_UNDO},//positive:1 increase 1 v operation;SEM_UNDO:
          {right,1,SEM_UNDO}
      };
 ...
 int main(int argc,char *argv[])
 {
	...
	while((pid=wait(&s))>0){//deal with case of zombie process
	...
	while (semctl (semid, 0, IPC_RMID, 1)<0) {//0 means we operate on all signals,IPC_RMID--delete the semaphores from the memory
		printf("Error in deallocationg semaphores.\n");
	}
	printf("Succeed in deallocationg semaphores.\n");
    return 0;
 }
