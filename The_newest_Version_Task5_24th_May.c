...
#define EATINGTIME 4
#define TOTALMEALTIME 21 //the total meal time
/*

Two philosophers who are not adjacent can eat at the same time. Therefore, 
in 4 seconds of eating time, two philosophers can eat at the same time. 
In 20 seconds, each philosopher can take turns to eat two meals. 
Similarly, in 40 seconds, philosophers can eat four meals. 
Here, 21s is set because the extra 1s is needed to print 
the last philosophers' eating information.

<1: the frist 5 seconds>philosopher[0] ,philosopher[2] eating

<2: the second 5 seconds>philosopher[1] ,philosopher[4] eating

<3: the third 5 seconds>philosopher[3] ,philosopher[0] eating

<4: the fourth 5 seconds>philosopher[2] ,philosopher[4] eating

<5: the fifth 5 seconds>philosopher[1] ,philosopher[3] eating
*/
int nuMeals=0; 
int num = 0;
...
//The second parameter is the semaphores set number
 ...
 int main(int argc,char *argv[])
 {
	...
	sleep(TOTALMEALTIME);//each philosopher 
